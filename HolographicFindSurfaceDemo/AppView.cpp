...
void AppView::Initialize(CoreApplicationView const& applicationView)
{
    ...
    //> Added by CurvSurf
    m_enteredBackgroundEventToken = CoreApplication::EnteredBackground(bind(&AppView::OnEnteredBackground, this, _1, _2));
    m_leavingBackgroundEventToken = CoreApplication::LeavingBackground(bind(&AppView::OnLeavingBackground, this, _1, _2));
    //< Added by CurvSurf [end]

    // At this point we have access to the device and we can create device-dependent
    // resources.
    m_deviceResources = std::make_shared<DX::DeviceResources>();

    m_main = std::make_unique<HolographicFindSurfaceDemoMain>(m_deviceResources);
}
...
void AppView::Uninitialize()
{
    ...
    //> Added by CurvSurf
    CoreApplication::EnteredBackground(m_enteredBackgroundEventToken);
    CoreApplication::LeavingBackground(m_leavingBackgroundEventToken);
    //< Added by CurvSurf [end]
...
//
// Added by CurvSurf (below codes)
//

void AppView::OnEnteredBackground(winrt::Windows::Foundation::IInspectable const& sender, winrt::Windows::ApplicationModel::EnteredBackgroundEventArgs const& args)
{
...
void AppView::OnLeavingBackground(winrt::Windows::Foundation::IInspectable const& sender, winrt::Windows::ApplicationModel::LeavingBackgroundEventArgs const& args)
{
    if (m_main != nullptr)
    {
        m_main->LeavingBackground();
    }
}